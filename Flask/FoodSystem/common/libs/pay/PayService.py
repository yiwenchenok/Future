...
from web.models.food.FoodSaleChangeLog import FoodSaleChangeLog
from web.models.payorder.PayOrder import PayOrder
from web.models.payorder.PayOrderItem import PayOrderItem
from web.models.payorder.PayOrderCallbackData import PayOrderCallbackData
...
class PayService():
    ...
    def orderSuccess(self, pay_order_id=0, params=None):  # 付款成功
        try:
            ...
            pay_order_info.status = 1  # 付款成功
            pay_order_info.express_status = -7  # -7： 已付款待发货
            pay_order_info.updated_time = getCurrentDate()
            db.session.add(pay_order_info)
            pay_order_items = PayOrderItem.query.filter_by(pay_order_id=pay_order_id).all()
